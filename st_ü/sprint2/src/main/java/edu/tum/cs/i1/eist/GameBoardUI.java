...
public class GameBoardUI extends Canvas implements Runnable {
	...
	public void steerCar() {
		...
		this.addMouseListener(mouseSteering);
	}
	...
	private void paintCar(Car car, Graphics graphics) {
		Point carPosition = car.getPosition();
		Point canvasPosition = convertPosition(carPosition);
		graphics.drawImage(car.getBody(), canvasPosition.x, canvasPosition.y, car.getSize().width, car.getSize().height, null);
	}

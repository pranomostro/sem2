...
import edu.tum.cs.i1.eist.car.OldCar;
...
public class GameBoard {
	...
	public static int NUMBER_OF_OLD_CARS=2;
	...
	public void addCars() {
		...
		for(int i=0; i<NUMBER_OF_OLD_CARS; i++) {
			cars.add(new OldCar(size.width, size.height));
		}
	}
	...
	public void moveCars() {
		...
		for (Car car : cars) {
			...
			Collision collision=new Collision(getPlayerCar(), car);
			if(collision.detectCollision()) {
				Car winner = collision.evaluate();
				System.out.println(winner);
				if(winner==getPlayerCar()) {
					car.setCrunched();
